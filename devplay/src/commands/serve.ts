...
export async function serveCommand(cwd: string, options: ServeOptions): Promise<void> {
  ...
  if (!existsSync(playgroundDir)) {
    ...
    log.info('Run "devplay build" or "devplay playground" first.');
    process.exit(1);
  }
    ...
    try {
      ...
      if (ext === '.html') {
        ...
        const injectedHtml = html.replace(
          '</body>',
          `<script>
            // devplay hot-reload (dev mode)
            let lastCheck = Date.now();
            setInterval(async () => {
              try {
                const res = await fetch('/__devplay_ping');
                if (res.ok) {
